# BungeeCord Netzwerk unter Linux einrichten
...
::: tip Vorher lesen
Java, Benutzer, Portfreigabe und Autostart findest du hier: [Grundlagen](/gameserver/minecraft/linux/grundlagen)
:::
...
- Mindestens zwei eingerichtete Backend-Server ([Paper](/gameserver/minecraft/linux/paper) empfohlen)
...
Beende den Proxy nach dem ersten Start mit `end`, damit die Konfiguration erzeugt wird.
...
Diese Abschnitte sind entscheidend:
...
Drei Maßnahmen:
1. `server-ip=127.0.0.1` setzen, damit nur lokale Verbindungen angenommen werden
2. Backend-Ports **nicht** in der Firewall freigeben — nur Port 25565 des Proxys
3. BungeeGuard einsetzen (siehe unten)

Prüfe mit `sudo ss -tulpn | grep 2556`, worauf deine Server tatsächlich lauschen.
:::
...
Mit systemd lässt sich das automatisieren — ergänze in der Proxy-Unit:

```ini
[Unit]
After=network.target minecraft-lobby.service minecraft-survival.service
```
...
- **Jeder kann sich als Admin einloggen:** Backend-Port ist öffentlich erreichbar. Sofort `ufw status` prüfen und BungeeGuard einrichten.
